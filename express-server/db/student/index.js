...
exports.addStudent = async ({ num, name, grade,_class ,address, sex }) => {
    name = name || ""
    grade = grade || ""
    _class = _class || ""
    address =address || ""
    ...
    let { err, results } = await query(sql, { 'student_num':num, 'student_name':name, 'student_grade':grade,'student_class':_class ,'student_address':address , 'student_sex':sex });
    // 执行 SQL 语句失败
...
exports.getStudents = async (page, limit, num, name, grade,_class,address , sex) => {
    ...
    if (address) {
        sql = sql + " student_address like ? and "
...
exports.getStudentsTotal = async (num, name, grade, _class ,address, sex) => {
...
exports.updateStudent = async ({ id, name, grade, _class ,address, sex }) => {
    sex = sex || 0
    name = name || ""
    grade = grade || ""
    _class = _class || ""
    ...
    let { err, results } = await query(sql, [ {  'student_name':name, 'student_grade':grade,'student_class':_class ,'student_address':address , 'student_sex':sex }, parseInt(id)]);
    if (err) return { err, isSuccess: false }
    // 执行 SQL 语句成功，但是影响行数不等于 1
    if (results.affectedRows !== 1) return { err: null, isSuccess: false }
    // 更新用户成功
